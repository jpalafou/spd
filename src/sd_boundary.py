...
def apply_BC(self,dim) -> None:
    ...
    if dim=="x":
        #nvar,nader,Nz,Ny,Nx+1,p+1,p+1
        if self.Z:
            self.dm.MR_fp_x[..., 0,:,:] = self.dm.BC_fp_x[0]
            self.dm.ML_fp_x[...,-1,:,:] = self.dm.BC_fp_x[1]
        elif self.Y:
            self.dm.MR_fp_x[..., 0,:] = self.dm.BC_fp_x[0]
            self.dm.ML_fp_x[...,-1,:] = self.dm.BC_fp_x[1]
        else:
        ...
        #nvar,nader,Nz,Ny+1,Nx,p+1,p+1
        if self.Z:
            self.dm.MR_fp_y[..., 0,:,:,:] = self.dm.BC_fp_y[0]
            self.dm.ML_fp_y[...,-1,:,:,:] = self.dm.BC_fp_y[1]
        else:
            self.dm.MR_fp_y[..., 0,:,:] = self.dm.BC_fp_y[0]
            self.dm.ML_fp_y[...,-1,:,:] = self.dm.BC_fp_y[1]
    elif dim=="z":
        #nvar,nader,Nz+1,Ny,Nx,p+1,p+1
        self.dm.MR_fp_z[..., 0,:,:,:,:] = self.dm.BC_fp_z[0]
        self.dm.ML_fp_z[...,-1,:,:,:,:] = self.dm.BC_fp_z[1]
